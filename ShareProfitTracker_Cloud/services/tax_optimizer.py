...
class IndianTaxOptimizer:
    ...
    def analyze_tax_situation(self, portfolio_data: Dict) -> Dict[str, Any]:
        """Synchronous method to analyze tax situation for GUI compatibility"""
        ...
        for holding in holdings:
            ...
            holding_info = {
                ...
                'days_to_ltcg': max(0, 365 - days_held) if days_held < 365 else 0
            }
            ...
            if days_held >= 365:
                if unrealized_gain > 0:
                    ...
                    long_term_losses += abs(unrealized_gain)
            else:
                if unrealized_gain > 0:
                    short_term_gains += unrealized_gain
                else:
                    short_term_losses += abs(unrealized_gain)

            holdings_analysis.append(holding_info)

        # Calculate tax liabilities
        stcg_tax = max(0, short_term_gains - short_term_losses) * 0.156  # 15.6%
        taxable_ltcg = max(0, long_term_gains - long_term_losses - 100000)  # 1 lakh exemption
        ltcg_tax = taxable_ltcg * 0.104  # 10.4%

        # Generate optimization strategies
        strategies = []

        # Tax-loss harvesting
        if short_term_losses > 0 or long_term_losses > 0:
            strategies.append({
                'name': 'Tax-Loss Harvesting',
                'description': 'Use losses to offset gains',
                'potential_savings': min(short_term_losses + long_term_losses, short_term_gains + long_term_gains) * 0.15
            })

        # Hold for LTCG
        for h in holdings_analysis:
            if not h['is_long_term'] and h['unrealized_gain'] > 0 and h['days_to_ltcg'] <= 90:
                strategies.append({
                    'name': f"Hold {h['symbol']} for LTCG",
                    'description': f"Wait {h['days_to_ltcg']} days for lower tax rate",
                    'potential_savings': h['unrealized_gain'] * 0.052  # Difference between STCG and LTCG
                })

        # LTCG exemption utilization
        if long_term_gains > 0 and long_term_gains <= 150000:
            strategies.append({
                'name': 'Utilize LTCG Exemption',
                'description': 'Book profits within ₹1 lakh exemption limit',
                'potential_savings': min(long_term_gains, 100000) * 0.104
            })
        ...
        return {
            ...
            'long_term': {
                ...
                'tax_rate': 0.104,
                'tax_liability': ltcg_tax
            },
            'total_tax_liability': stcg_tax + ltcg_tax,
            'holdings_analysis': holdings_analysis,
            'optimization_strategies': strategies,
            'recommendations': self._generate_recommendations(holdings_analysis, short_term_gains, long_term_gains)
        }

    def _generate_recommendations(self, holdings, stcg, ltcg):
        """Generate tax optimization recommendations"""
        recommendations = []

        if stcg > 100000:
            recommendations.append("Consider holding high-gain stocks for 1 year to benefit from lower LTCG rates")

        if ltcg > 100000:
            recommendations.append("Book profits strategically to utilize ₹1 lakh LTCG exemption annually")

        # Find stocks close to LTCG threshold
        for h in holdings:
            if not h['is_long_term'] and h['days_to_ltcg'] <= 30 and h['unrealized_gain'] > 10000:
                recommendations.append(f"{h['symbol']}: Wait {h['days_to_ltcg']} days for LTCG benefit")

        if not recommendations:
            recommendations.append("Your portfolio is well-optimized for tax efficiency")
