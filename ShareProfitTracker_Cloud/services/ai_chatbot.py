...
class FinancialAIAdvisor:
    ...
    def get_advice(self, question: str, portfolio_data: Optional[Dict] = None) -> str:
        """Synchronous wrapper for getting AI advice - for GUI compatibility"""
        import asyncio
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(asyncio.run, self.chat_with_advisor(question, portfolio_data))
                    return future.result(timeout=30)
            else:
                return asyncio.run(self.chat_with_advisor(question, portfolio_data))
        except Exception as e:
            # Fallback to simple response if async fails
            return self._generate_simple_advice(question, portfolio_data)

    def _generate_simple_advice(self, question: str, portfolio_data: Optional[Dict] = None) -> str:
        """Generate simple advice without AI service"""
        ...
        if portfolio_data:
            ...
            if 'perform' in question_lower or 'how' in question_lower:
                pnl_percent = (total_pnl / (total_value - total_pnl) * 100) if total_value > total_pnl else 0
                status = "profit" if total_pnl > 0 else "loss"
                return f"Your portfolio has {num_stocks} stocks with a total value of ₹{total_value:,.2f}. You're currently in {status} of ₹{abs(total_pnl):,.2f} ({abs(pnl_percent):.1f}%). Consider reviewing underperforming stocks and maintaining diversification."

            if 'rebalance' in question_lower:
                return f"With {num_stocks} stocks in your portfolio, consider rebalancing if any single stock exceeds 20% of your total holdings. Review sector allocation and consider your risk tolerance."

            if 'tax' in question_lower:
                return "For tax optimization, consider holding stocks for more than 1 year to benefit from lower LTCG rates (10.4% vs 15.6% for STCG). You can also offset gains with losses through tax-loss harvesting."

            if 'risk' in question_lower:
                return f"Your portfolio risk depends on diversification across {num_stocks} stocks. Ensure no single stock exceeds 15-20% of portfolio. Consider your investment horizon and risk tolerance."

        return "I can help you analyze your portfolio, provide tax advice, suggest rebalancing, and assess risk. Please ask a specific question about your investments."
